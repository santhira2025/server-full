...
use crate::bpf::{attach_kernel_probes, attach_tls_uprobes};
...
struct Args {
    ...
    #[arg(
        long,
        value_delimiter = ',',
        default_value = "/usr/lib/x86_64-linux-gnu/libssl.so.3"
    )]
    tls_libs: Vec<String>,
...
async fn main() -> Result<()> {
    ...
    let http_client = Arc::new(
        reqwest::Client::builder()
            ...
            .user_agent("api-sentinel-sensor/0.1.0")
            .build()?,
    );
    ...
    // DNS resolver
    let dns_resolver = if args.dns_resolve {
        Some(Arc::new(DnsResolver::new(args.dns_ttl, args.dns_negative_ttl)))
    } else {
        None
    };
    ...
    if let Some(proc_map) = proc_events_result {
        ...
        unsafe {
            let _ = ringbuf.add(&*proc_map_ptr, move |data| {
