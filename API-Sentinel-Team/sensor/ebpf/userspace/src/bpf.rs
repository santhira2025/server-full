...
pub fn attach_kernel_probes(
    ...
    links.push(
        tcp_accept
            .attach()
            .context("attach kretprobe inet_csk_accept")?,
    );
    ...
    // UDP/QUIC probes (optional — may not exist in BPF object)
    if let Some(prog) = obj.prog_mut("udp_sendmsg_entry") {
        match prog.attach() {
            Ok(link) => {
                links.push(link);
                tracing::info!("attached kprobe udp_sendmsg");
            }
            Err(e) => tracing::debug!(error = %e, "skipping kprobe udp_sendmsg"),
        }
    }
    if let Some(prog) = obj.prog_mut("udp_sendmsg_exit") {
        match prog.attach() {
            Ok(link) => {
                links.push(link);
                tracing::info!("attached kretprobe udp_sendmsg");
            }
            Err(e) => tracing::debug!(error = %e, "skipping kretprobe udp_sendmsg"),
        }
    }
    if let Some(prog) = obj.prog_mut("udp_recvmsg_entry") {
        match prog.attach() {
            Ok(link) => {
                links.push(link);
                tracing::info!("attached kprobe udp_recvmsg");
            }
            Err(e) => tracing::debug!(error = %e, "skipping kprobe udp_recvmsg"),
        }
    }
    if let Some(prog) = obj.prog_mut("udp_recvmsg_exit") {
        match prog.attach() {
            Ok(link) => {
                links.push(link);
                tracing::info!("attached kretprobe udp_recvmsg");
            }
            Err(e) => tracing::debug!(error = %e, "skipping kretprobe udp_recvmsg"),
        }
    }
