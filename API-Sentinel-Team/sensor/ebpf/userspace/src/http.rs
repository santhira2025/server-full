...
pub fn extract_http_header(buf: &[u8]) -> Option<(HttpMessage, Vec<u8>)> {
    ...
    let header_str = match std::str::from_utf8(header_bytes) {
        ...
        Err(_) => {
            ...
            return Some((
                HttpMessage::Request(HttpRequestParsed {
    ...
    if first.starts_with("HTTP/") {
        ...
        return Some((
            HttpMessage::Response(HttpResponseParsed {
                status_code: status,
                headers,
            }),
            remaining,
...
fn advance_past_body(headers: &HashMap<String, String>, buf: &[u8], body_start: usize) -> Vec<u8> {
