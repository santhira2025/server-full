...
use crate::go_tls::{attach_at_offset, find_elf_symbol, find_elf_symbol_dyn, va_to_file_offset};
...
pub fn attach_boring_ssl_static(
    ...
    if !is_boring_ssl(&elf_file, &data) {
        return false;
    }
    ...
    let resolve = |name: &str| -> Option<usize> {
        let va =
            find_elf_symbol(&elf_file, name).or_else(|| find_elf_symbol_dyn(&elf_file, name))?;
