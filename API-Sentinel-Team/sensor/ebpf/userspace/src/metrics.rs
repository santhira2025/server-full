...
apisec_active_connections {active_conns}

# HELP apisec_ringbuf_drops_total Kernel ring buffer drops
# TYPE apisec_ringbuf_drops_total counter
apisec_ringbuf_drops_total {ringbuf_drops}

# HELP apisec_protocol_events_total Events by protocol
# TYPE apisec_protocol_events_total counter
apisec_protocol_events_total{{protocol=\"http1\"}} {p_http1}
apisec_protocol_events_total{{protocol=\"http2\"}} {p_http2}
apisec_protocol_events_total{{protocol=\"grpc\"}} {p_grpc}
...
",
        ...
        active_conns = ACTIVE_CONNECTIONS.load(Ordering::Relaxed),
        ringbuf_drops = RINGBUF_DROPS.load(Ordering::Relaxed),
        p_http1 = PROTO_HTTP1.load(Ordering::Relaxed),
        p_http2 = PROTO_HTTP2.load(Ordering::Relaxed),
        p_grpc = PROTO_GRPC.load(Ordering::Relaxed),
